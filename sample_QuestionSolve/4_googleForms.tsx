...
return (
  <Box sx={{ bgcolor: '#f1f3f4', minHeight: '100vh' }}>
    {/* 상단 앱바 */}
    <Box sx={{ bgcolor: 'white', borderBottom: '1px solid #dadce0', position: 'sticky', top: 0, zIndex: 100 }}>
      <Box sx={{ maxWidth: 900, mx: 'auto', py: 2, px: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
    ...
    {/* 메인 */}
    <Box sx={{ maxWidth: 768, mx: 'auto', py: 4, px: 3 }}>
      <Paper elevation={0} sx={{ p: 4, borderRadius: 2, bgcolor: 'white', border: '1px solid #dadce0' }}>
        {/* 헤더 */}
        <Box sx={{ mb: 3, pb: 2, borderBottom: '2px solid #673ab7', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="body1" sx={{ color: '#5f6368' }}>
            문제 {currentQuestionIndex + 1} / {parsedData.questions.length}
          </Typography>
          <Box sx={{ width: 200, height: 4, bgcolor: '#e8eaed', borderRadius: 2, overflow: 'hidden' }}>
            <Box sx={{ height: '100%', width: `${((currentQuestionIndex + 1) / parsedData.questions.length) * 100}%`, bgcolor: '#673ab7', transition: 'width 0.3s' }} />
          </Box>
        </Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Button onClick={handlePrevQuestion} disabled={currentQuestionIndex === 0} startIcon={<ArrowBackIcon />} sx={{ textTransform: 'none', color: '#1a73e8', fontWeight: 500 }}>
            이전
          </Button>
          
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
            <Typography variant="h6" fontWeight={600} sx={{ color: '#1a73e8' }}>
              {currentQuestionIndex + 1} / {parsedData.questions.length}
            </Typography>
            {!showResult ? (
              <Button variant="contained" onClick={handleCheckResult} disabled={isCheckButtonDisabled} sx={{ px: 4, py: 1, borderRadius: 1, textTransform: 'none', fontWeight: 500, bgcolor: '#1a73e8', boxShadow: 'none' }}>
                제출
              </Button>
            ) : (
              <Button variant="contained" onClick={handleNextQuestion} disabled={currentQuestionIndex === parsedData.questions.length - 1} sx={{ px: 4, py: 1, borderRadius: 1, textTransform: 'none', fontWeight: 500, bgcolor: '#1a73e8' }}>
                다음
              </Button>
            )}
          </Box>
    ...
    {/* 결과 */}
    {showResult && (
      <Box sx={{ maxWidth: 768, mx: 'auto', px: 3, pb: 4 }}>
        <Paper elevation={2} sx={{ p: 3, bgcolor: isCorrect ? '#e6f4ea' : '#fce8e6', border: `1px solid ${isCorrect ? '#34a853' : '#ea4335'}` }}>
          <Typography variant="h6" sx={{ color: isCorrect ? '#34a853' : '#ea4335', mb: 2 }}>
            {isCorrect ? '정답입니다!' : '오답입니다!'}
          </Typography>
          <Card>
            <CardContent>
              <Typography variant="body2">{currentQuestion.explanation || '해설이 없습니다.'}</Typography>
            </CardContent>
          </Card>
        </Paper>
      </Box>
