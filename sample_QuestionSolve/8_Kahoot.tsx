...
return (
  <Box sx={{ bgcolor: '#46178f', minHeight: '100vh' }}>
    <Box sx={{ maxWidth: 900, mx: 'auto', py: 3, px: 3 }}>
      {/* 상단 */}
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h5" sx={{ color: 'white', fontWeight: 800 }}>
          QUIZ TIME! 🎮
        </Typography>
        <Button onClick={onClose} sx={{ bgcolor: 'white', color: '#46178f', fontWeight: 800, px: 3, '&:hover': { bgcolor: '#f0f0f0' } }}>
          나가기
        </Button>
      </Box>
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
        <Paper sx={{ flex: 1, p: 2, bgcolor: '#ff3355', color: 'white', textAlign: 'center', borderRadius: 2 }}>
          <Typography variant="h4" fontWeight={800}>{currentQuestionIndex + 1}/{parsedData.questions.length}</Typography>
          <Typography variant="caption">문제</Typography>
        </Paper>
      </Box>

      {/* 문제 카드 */}
      <Paper elevation={8} sx={{ p: 5, borderRadius: 3, bgcolor: 'white', minHeight: 400, mb: 3, border: '4px solid #fff' }}>
        <Typography variant="h5" fontWeight={800} sx={{ color: '#46178f', mb: 3 }}>
      ...
      <Box sx={{ display: 'flex', gap: 2 }}>
        <Button variant="contained" onClick={handlePrevQuestion} disabled={currentQuestionIndex === 0} fullWidth sx={{ py: 3, bgcolor: '#26890d', fontSize: '1.2rem', fontWeight: 800, borderRadius: 2 }}>
          이전
        </Button>
        {!showResult ? (
          <Button variant="contained" onClick={handleCheckResult} disabled={isCheckButtonDisabled} fullWidth sx={{ py: 3, bgcolor: '#ff3355', fontSize: '1.2rem', fontWeight: 800, borderRadius: 2 }}>
            제출!
          </Button>
        ) : (
          <Button variant="contained" onClick={handleNextQuestion} disabled={currentQuestionIndex === parsedData.questions.length - 1} fullWidth sx={{ py: 3, bgcolor: '#ffa602', fontSize: '1.2rem', fontWeight: 800, borderRadius: 2 }}>
            다음!
          </Button>
        )}
      </Box>
    ...
    {showResult && (
      <Box sx={{ maxWidth: 900, mx: 'auto', px: 3 }}>
        <Paper sx={{ p: 5, bgcolor: isCorrect ? '#26890d' : '#e21b3c', color: 'white', borderRadius: 3, textAlign: 'center' }}>
          <Typography variant="h3" fontWeight={800} sx={{ mb: 2 }}>
            {isCorrect ? '🎉 정답!' : '😢 오답!'}
          </Typography>
          <Typography variant="h6">{currentQuestion.explanation || '해설 없음'}</Typography>
        </Paper>
      </Box>
    )}
  </Box>
