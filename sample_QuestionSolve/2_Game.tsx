...
export default function QuestionSolver({ questionItem, onClose }: QuestionSolverProps) {
  ...
  <Box sx={{ bgcolor: '#f8fafc', minHeight: '100vh', pb: 4 }}>
    ...
    >
      <Box sx={{ maxWidth: 1000, mx: 'auto', py: 2.5, px: 3, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        {/* 좌측: 제목 */}
      ...
      {/* 프로그레스 바 */}
      <Box sx={{ maxWidth: 1000, mx: 'auto', px: 3, pb: 1.5 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Box sx={{ flex: 1, height: 10, bgcolor: '#e2e8f0', borderRadius: 5, overflow: 'hidden' }}>
            <Box
              sx={{
                height: '100%',
                width: `${((currentQuestionIndex + 1) / parsedData.questions.length) * 100}%`,
                bgcolor: '#3b82f6',
                borderRadius: 5,
    ...
    <Box sx={{ maxWidth: 800, mx: 'auto', px: 3, py: 4 }}>
      {/* 문제 정보 카드 */}
      <Paper elevation={0} sx={{ p: 2, mb: 3, borderRadius: 2, bgcolor: '#eff6ff', border: '1px solid #bfdbfe' }}>
        <Typography variant="body2" color="text.secondary">
          📄 파일명: <strong>{questionItem.name}</strong>
        </Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
          🎯 문제 유형: <strong>{questionItem.displayType}</strong>
        </Typography>
      </Paper>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 4 }}>
        <Button
          ...
          sx={{
            ...
            borderColor: '#e2e8f0',
            color: '#64748b',
            '&:hover': { borderWidth: 2, borderColor: '#cbd5e0', bgcolor: '#f8fafc' },
            '&:disabled': { borderColor: '#f1f5f9', color: '#cbd5e0' }
          }}
        >
          이전 문제
        </Button>
        ...
        {/* 중앙: 페이지 번호 */}
        <Box sx={{ display: 'flex', alignItems: 'center', px: 3, py: 1, bgcolor: '#eff6ff', borderRadius: 3, border: '2px solid #3b82f6' }}>
          <Typography variant="h4" fontWeight={800} sx={{ color: '#3b82f6' }}>
            {currentQuestionIndex + 1}
          </Typography>
          <Typography variant="h5" sx={{ color: '#94a3b8', mx: 1.5 }}>/</Typography>
    ...
    {showResult && (
      <Box sx={{ maxWidth: 800, mx: 'auto', px: 3 }}>
        <Paper elevation={6} sx={{ p: 4, borderRadius: 3, bgcolor: isCorrect ? '#ecfdf5' : '#fef2f2', border: `2px solid ${isCorrect ? '#10b981' : '#ef4444'}` }}>
          <Typography variant="h5" fontWeight={700} sx={{ color: isCorrect ? '#059669' : '#dc2626', mb: 3 }}>
            {isCorrect ? '✅ 정답입니다!' : '❌ 오답입니다!'}
          </Typography>
          <Divider sx={{ my: 2 }} />
          <Box sx={{ bgcolor: 'white', p: 3, borderRadius: 2 }}>
            <Typography variant="h6" fontWeight={700} gutterBottom>💡 해설</Typography>
