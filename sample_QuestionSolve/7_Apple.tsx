...
return (
  <Box sx={{ bgcolor: '#f5f5f7', minHeight: '100vh' }}>
    {/* 헤더 */}
    <Box sx={{ bgcolor: 'rgba(255,255,255,0.8)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(0,0,0,0.1)', position: 'sticky', top: 0, zIndex: 100 }}>
      <Box sx={{ maxWidth: 800, mx: 'auto', py: 2, px: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Typography variant="h6" sx={{ color: '#1d1d1f', fontWeight: 600, letterSpacing: '-0.5px' }}>
          문제 풀기
        </Typography>
        <Button onClick={onClose} sx={{ textTransform: 'none', color: '#06c', fontWeight: 600 }}>
          완료
        </Button>
      </Box>
    </Box>
    ...
    <Box sx={{ maxWidth: 680, mx: 'auto', py: 6, px: 3 }}>
      {/* 프로그레스 */}
      <Box sx={{ mb: 4 }}>
        <Box sx={{ height: 2, bgcolor: '#d2d2d7', borderRadius: 1, overflow: 'hidden' }}>
          <Box sx={{ height: '100%', width: `${((currentQuestionIndex + 1) / parsedData.questions.length) * 100}%`, bgcolor: '#06c', transition: 'width 0.5s ease' }} />
        </Box>
        <Typography variant="caption" sx={{ color: '#86868b', mt: 1, display: 'block', textAlign: 'center' }}>
          {currentQuestionIndex + 1} / {parsedData.questions.length}
        </Typography>
      </Box>

      {/* 문제 */}
      <Paper elevation={0} sx={{ p: 4, borderRadius: 3, bgcolor: 'white', mb: 4, border: '1px solid #d2d2d7' }}>
        {renderQuestionComponent()}
      </Paper>

      {/* 버튼 */}
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 4 }}>
        <Button onClick={handlePrevQuestion} disabled={currentQuestionIndex === 0} sx={{ textTransform: 'none', color: '#06c', fontWeight: 600 }}>
          이전
        </Button>
        
        <Box sx={{ px: 4, py: 1.5, bgcolor: '#f5f5f7', borderRadius: 2 }}>
          <Typography variant="h5" fontWeight={600} sx={{ color: '#1d1d1f' }}>
            {currentQuestionIndex + 1} / {parsedData.questions.length}
          </Typography>
        </Box>

        {!showResult ? (
          <Button variant="contained" onClick={handleCheckResult} disabled={isCheckButtonDisabled} sx={{ textTransform: 'none', bgcolor: '#06c', px: 5, py: 1.5, borderRadius: 2, fontWeight: 600, boxShadow: 'none', '&:hover': { bgcolor: '#0077ed', boxShadow: 'none' } }}>
            제출
          </Button>
        ) : (
          <Button variant="contained" onClick={handleNextQuestion} disabled={currentQuestionIndex === parsedData.questions.length - 1} sx={{ textTransform: 'none', bgcolor: '#06c', px: 5, py: 1.5, borderRadius: 2, fontWeight: 600 }}>
            다음
          </Button>
        )}
      </Box>
    ...
    {showResult && (
      <Box sx={{ maxWidth: 680, mx: 'auto', px: 3, pb: 6 }}>
        <Paper sx={{ p: 4, bgcolor: isCorrect ? '#f0fdf4' : '#fef2f2', borderRadius: 3, border: `1px solid ${isCorrect ? '#86efac' : '#fecaca'}` }}>
          <Typography variant="h6" fontWeight={600} sx={{ color: isCorrect ? '#16a34a' : '#dc2626', mb: 2 }}>
            {isCorrect ? '정답' : '오답'}
          </Typography>
          <Typography variant="body2" sx={{ color: '#1d1d1f', lineHeight: 1.7 }}>
