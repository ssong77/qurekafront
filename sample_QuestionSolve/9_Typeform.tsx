...
return (
  <Box sx={{ bgcolor: '#191919', minHeight: '100vh', color: 'white', display: 'flex', flexDirection: 'column' }}>
    {/* 상단 */}
    <Box sx={{ p: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <Typography sx={{ color: '#999' }}>
        {currentQuestionIndex + 1} / {parsedData.questions.length}
      </Typography>
      <Button onClick={onClose} sx={{ color: 'white' }}>✕ 닫기</Button>
    </Box>

    {/* 프로그레스 */}
    <Box sx={{ px: 3 }}>
      <Box sx={{ height: 2, bgcolor: '#333', overflow: 'hidden' }}>
        <Box sx={{ height: '100%', width: `${((currentQuestionIndex + 1) / parsedData.questions.length) * 100}%`, bgcolor: 'white', transition: 'width 0.5s' }} />
      </Box>
    </Box>
    ...
    <Box sx={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', px: 3 }}>
      <Box sx={{ maxWidth: 700, width: '100%' }}>
        ...
        <Box sx={{ bgcolor: '#262626', p: 5, borderRadius: 3, border: '1px solid #333' }}>
    ...
    <Box sx={{ p: 3, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 4 }}>
      <Button onClick={handlePrevQuestion} disabled={currentQuestionIndex === 0} sx={{ color: 'white' }}>
        ↑ 이전
      </Button>
      ...
      {!showResult ? (
        <Button variant="contained" onClick={handleCheckResult} disabled={isCheckButtonDisabled} sx={{ bgcolor: 'white', color: '#191919', px: 4, py: 1.5, fontWeight: 700, '&:hover': { bgcolor: '#f0f0f0' } }}>
          제출 ↵
        </Button>
      ) : (
        <Button variant="contained" onClick={handleNextQuestion} disabled={currentQuestionIndex === parsedData.questions.length - 1} sx={{ bgcolor: 'white', color: '#191919', px: 4, py: 1.5, fontWeight: 700 }}>
          다음 ↓
        </Button>
      )}
    </Box>
    ...
    {showResult && (
      <Box sx={{ position: 'fixed', bottom: 100, left: 0, right: 0, px: 3 }}>
        <Box sx={{ maxWidth: 700, mx: 'auto', p: 4, bgcolor: isCorrect ? '#10b981' : '#ef4444', borderRadius: 3 }}>
          <Typography variant="h5" fontWeight={700} sx={{ mb: 2 }}>
            {isCorrect ? '✅ 정답!' : '❌ 오답!'}
          </Typography>
          <Typography>{currentQuestion.explanation || '해설 없음'}</Typography>
        </Box>
      </Box>
    )}
  </Box>
