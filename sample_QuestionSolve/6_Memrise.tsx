  ...
  <Box sx={{ bgcolor: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', minHeight: '100vh' }}>
    <Box sx={{ maxWidth: 800, mx: 'auto', py: 4, px: 3 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Box sx={{ flex: 1 }}>
          <Box sx={{ height: 12, bgcolor: 'rgba(255,255,255,0.3)', borderRadius: 6, overflow: 'hidden' }}>
            <Box sx={{ height: '100%', width: `${((currentQuestionIndex + 1) / parsedData.questions.length) * 100}%`, bgcolor: '#fbbf24', borderRadius: 6, transition: 'width 0.5s' }} />
          </Box>
        </Box>
        <Button onClick={onClose} sx={{ ml: 2, color: 'white', fontWeight: 700 }}>✕</Button>
      </Box>
      ...
      {/* 문제 카드 */}
      <Paper elevation={16} sx={{ p: 5, borderRadius: 4, bgcolor: 'white', minHeight: 350, mb: 3 }}>
      ...
      <Box sx={{ display: 'flex', gap: 2 }}>
        <Button variant="outlined" onClick={handlePrevQuestion} disabled={currentQuestionIndex === 0} fullWidth sx={{ py: 2, borderRadius: 3, borderColor: 'white', color: 'white', borderWidth: 2, fontWeight: 700, '&:hover': { borderWidth: 2 } }}>
          뒤로
        </Button>
        {!showResult ? (
          <Button variant="contained" onClick={handleCheckResult} disabled={isCheckButtonDisabled} fullWidth sx={{ py: 2, borderRadius: 3, bgcolor: '#fbbf24', color: '#000', fontWeight: 700, fontSize: '1.1rem' }}>
            체크
          </Button>
        ) : (
          <Button variant="contained" onClick={handleNextQuestion} disabled={currentQuestionIndex === parsedData.questions.length - 1} fullWidth sx={{ py: 2, borderRadius: 3, bgcolor: '#10b981', fontWeight: 700, fontSize: '1.1rem' }}>
            계속
          </Button>
        )}
      </Box>
    ...
    {showResult && (
      <Box sx={{ maxWidth: 800, mx: 'auto', px: 3 }}>
        <Paper sx={{ p: 4, bgcolor: isCorrect ? '#d1fae5' : '#fee2e2', borderRadius: 3 }}>
          <Typography variant="h5" fontWeight={700}>{isCorrect ? '👍 잘했어요!' : '👎 틀렸어요'}</Typography>
          <Typography sx={{ mt: 2 }}>{currentQuestion.explanation || '해설 없음'}</Typography>
        </Paper>
      </Box>
    )}
  </Box>
